...
BATCH_SIZE = 64
MEMORY_CAPACITY = 128
GAME_STEP_NUM = iter_ - 3  # (for the zero)
EPOCHS = 5000
...
# Network parameters
input_dim = Total_map_number + Small_map_number  # Q() = S{total} x V{small}
hidden_dim = 16
output_dim = 1
Reward_hit = 10
Reward_fetch = 20.0
...
class DQN(object):
    def __init__(self):
        ...
        self.memory = torch.zeros((MEMORY_CAPACITY, N_S_A * 3 + 2)).to(device=cuda0)  # initialize memory ( S + A + 'r' + S_ + 'g_step')
    ...
    def learn(self):
        ...
        for i in range(BATCH_SIZE):
            vec_batch_.append(vec_num[int(g_step[i])])  # V_t+1

        vec_batch_ = torch.tensor(vec_batch_, dtype=torch.float32).to(device=cuda0)

        In_q_eval = torch.cat((x_, vec_batch_), 1)
        '''
        ...
        q_next = GetTargetS_(b_s_, dqn.target_net, g_step).to(device=cuda0)  # float.Tensor 16
...
for i_episode in range(EPOCHS):
    ...
    while True:
        ...
        if dqn.memory_counter > MEMORY_CAPACITY:
            loss = dqn.learn()
            if done:
                log_.info('Ep: %s'  '| Ep_loss: %f', for_log_timer, loss)
                log_.info('Ep: %s'  '| Ep_r: %f', for_log_timer, round(ep_r, 2))
                # Save every 5 eps
                if for_log_timer % 5 == 0:
                    file_num = str(int(for_log_timer / 5))
                    eval_name = 'saved/eval_network_' + file_num + ".pt"
                    target_name = 'saved/target_network_' + file_num + ".pt"
                    torch.save(dqn.eval_net.state_dict(), eval_name)
                    # print('Save eval_network complete.')
                    torch.save(dqn.target_net.state_dict(), target_name)
                    # print('Save target_network complete.')
                    log_.info("Saved %s succeed", file_num)
                for_log_timer += 1
        if done:
            break
        s = s_
