...
class DQN(object):
    def __init__(self):
        ...
        self.memory = np.zeros((MEMORY_CAPACITY, N_S_A * 2 + 1 + N_S_A + 1))  # initialize memory ( S + A + r + S_ + g_step )
    ...
    def learn(self):
        ...
        '''
        try:
            q_eval = GetEvalS_(b_s, dqn.eval_net, vec_num, g_step)  # float.Tensor
        except:
            print("Stop")
            sys.exit()
        q_next = GetTargetS_(b_s_, dqn.target_net, vec_num, g_step)  # float.Tensor
        # Here
...
def GetEvalS_(s_, eval_net, vec_num_in, g_step):
    ...
    for i in range(BATCH_SIZE):
        gurobi_func = gb.Gurobi(fc1_weight, fc1_bias, fc2_weight, fc2_bias, vec_num_in[g_step[i]],
...
def GetTargetS_(s_new, target_net, vec_num_in, g_step):
    ...
    for i in range(BATCH_SIZE):
        ...
        if game_step == GAME_STEP_NUM:
            ...
            game_step += 1
            gurobi_func = gb.Gurobi(fc1_weight, fc1_bias, fc2_weight, fc2_bias, vec_num_in[g_step[game_step]],
...
for i_episode in range(EPOCHS):
    ...
    while True:
        ...
        game_step = Game_step
        dqn.store_transition(s, a, r, s_, game_step)
