...
class DQN(object):
    def __init__(self):
        ...
        try:
            self.eval_net.load_state_dict(torch.load('./saved/eval_network_17.pt'))
            self.target_net.load_state_dict(torch.load('./saved/target_network_17.pt'))
        ...
        self.memory = torch.zeros((MEMORY_CAPACITY, )).to(device=cuda0)  # initialize memory ( [S,V] + A + 'r' + S_)
