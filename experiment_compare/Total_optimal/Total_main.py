...
def choose_action(observation_state, vec_num_in):
    ...
    gurobi_func = gb.Gurobi(vec_num_in,
...
for it in range(iter_):
    print("a[it][j]: \n")
    for j in range(Total_map_number):  # From map j in time i
        print(a[it][j], end=" ")
    print()
...
for it in range(iter_):
    for j in range(Small_map_number):  # From map j in time i
        if a[it][j] == 1:  # Cached by Small m
            continue
        else:
            if a[it][Small_map_number + map_[j]] == 1:  # Cached by Big M
                reward_temp += Reward_hit * vec_num[it][j]
            else:  # Fail, and fetching from data set
                reward_temp += Reward_fetch * vec_num[it][j]
    reward_total += reward_temp
