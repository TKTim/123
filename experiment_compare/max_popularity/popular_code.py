    ...
    def getReward(self):
        ...
        for j in range(Small_map_number):  # From map j in time i
            if j in self.states:
                ...
                reward_temp += Reward_fetch * re_num[Game_step][j]
...
def choose_action(observation_state):
    ...
    # print("formal_list\n", formal_list)
    a_in = sorted(formal_list, key=lambda s: s[1], reverse=True)
    print("a_in\n", a_in)
...
'''
# ptr_re
lines = (line.rstrip() for line in ptr_re.readlines())  # All lines including the blank ones. Skip first line.
lines = (line for line in lines if line)  # Non-blank lines

re_num = [[0.0 for _ in range(Small_map_number)] for _ in range(iter_)]  # numbers of maps
    ...
    for i in line:
        ...
        if search_:  # Starting to search everything
            if i == "]":  # End of the line
                ...
                try:
                    re_num[iter_time][map_pos] = float(temp)
                except:
                    print("iter_time, map_pos  ", iter_time, map_pos)
                map_pos += 1
                temp = ""
