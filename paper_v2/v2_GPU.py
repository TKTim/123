...
def get_state_action():
    print("get_state")
    gen_state = np.random.choice([0, 1], size=N_S_A, p=[0.65, 0.35])
    count = 0
    for i in gen_state:
        if i == 1:
            count += 1
    ...
    def choose_action(self, observation_state, vec_num_in):
        ...
        if np.random.uniform() < EPSILON:  # Gurobi Solver
            ...
            gurobi_func = gb.Gurobi(fc1_weight, fc1_bias, fc2_weight, fc2_bias, vec_num_in[Game_step],
                                    observation_state, B_size=B_size, D_size=D_size, Reward_hit=Reward_hit,
                                    Reward_fetch=Reward_fetch, Total_map=Total_map_number, Small_number=Small_map_number
                                    , map_=map_)
            action = gurobi_func.MIP_formulation()
            print("[ ", end="", file=ptr_ouput)
            for i in action:
                print(i, " ", end="", file=ptr_ouput)
            print("]", end="", file=ptr_ouput)
            if self.timer_output_Action != 5:
                self.timer_output_Action += 1
            else:
                ptr_ouput.close()
                sys.exit()
        else:  # random
...
for i_episode in range(EPOCHS):
    ...
    while True:
        ...
        print("Reward: ",r, end="", file=ptr_ouput)
