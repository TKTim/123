...
class Gurobi:
    def __init__(self, fc1_weight, fc1_bias, fc2_weight, fc2_bias, vec_num, t_minus_s_, Reward_hit, Reward_fetch,
        ...
        self.B_size = B_size
        self.map_ = map_
        self.Reward_hit_positive = Reward_hit * -1
        self.Reward_fetch_positive = Reward_fetch * -1
    ...
    def MIP_formulation(self):
        ...
        # Create variables
        s_ = m.addVars(range(self.Total_map), ub=1, lb=0, vtype=GRB.BINARY, name='s_')
        x_ = m.addVars(range(self.Total_map), ub=1, lb=0, vtype=GRB.BINARY, name='re_s_')
        hit = m.addVars(range(self.Small_number), ub=1, lb=0, vtype=GRB.BINARY, name='hit_')
        fetch = m.addVars(range(self.Small_number), ub=1, lb=0, vtype=GRB.BINARY, name='fetch_')
        d_m_ = m.addVars(range(self.Total_map), ub=1, lb=0, vtype=GRB.BINARY, name='d_m_')
        ...
        for i in range(self.Small_number):
            try:
                m.addConstr(hit[i] == and_(x_[i], s_[self.Small_number + self.map_[i]]), "hit_")  # (m is not) and (M is) <hit>
                m.addConstr(fetch[i] == and_(x_[i], x_[self.Small_number + self.map_[i]]), "fetch_")
            except:
                print("Fail H F.")
                print("map_: ", self.map_[i])
                sys.exit()
