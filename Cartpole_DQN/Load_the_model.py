...
class DQN(object):
    def __init__(self):
        ...
        self.eval_net.load_state_dict(torch.load('eval_network_v2.pt'))
        self.target_net.load_state_dict(torch.load('target_network_v2.pt'))
    ...
    while True:
        ...
        if dqn.memory_counter > MEMORY_CAPACITY:
            ...
            if done:
                print('Ep: ', i_episode,
                      '| Ep_r: ', round(ep_r, 2))
                reward_plot.append(ep_r)
                reward_num.append(i_episode)
...
plt.plot(reward_num, reward_plot)


torch.save(dqn.eval_net.state_dict(), 'eval_network_v3.pt')
print('Save eval_network complete.')
torch.save(dqn.target_net.state_dict(), 'target_network_v3.pt')
