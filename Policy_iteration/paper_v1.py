...
for i in range(max_policy_iter):
    ...
    for j in range(max_value_iter):
        ...
        for s in states:
            ...
            # Compute state value for state s
            val = rewards[s]  # Get direct reward

            # Need to use NN to solve [START]
            for s_next in states:
                prob_now = probs[s][s_next][pi[s]]
                action_temp = pi[s]
                val += prob_now * (
                        gamma * V[s_next]
                )  # Add discounted downstream values
            # Need to use NN to solve [END]
        ...
        # print out process
        print('policy iteration: ', i, ',  value iteration: ', j, ',  diff: ', max_diff)
