...
FILE *fl_input = NULL, *fl_output = NULL;
...
void carListTraverse(struct linked_list *car_head, int msg_num, FILE *fl_ptr) //traverse the car lists and display their elements into 'fl_ptr' file.
{
	...
	else
	{
		...
		sect_list = getVehSect(linked_list_ptr);
		// Link...
		
		if(flag_list == NULL){
			flag_list_tail = listIdInsert(&flag_list, flag_list_tail, sect_list->id);
		}else{
			flag_list_tail->next = sect_list;
			flag_list_tail = getListTail(sect_list);
		}
		printf("flag: %d\n", getListSize(flag_list));
		/*
		printList(flag_list);
		printf("\n");
		*/
		...
		sect_mapping_tb = (int *) malloc(sizeof(int) * msg_num);
		

		for(int i = 0; i < msg_num; i ++) //construct the mapping.
		{
			sect_mapping_tb[i] = sect_list_ptr->id;
			// printf("%d, ",sect_mapping_tb[i]);
		...
		{
			//linked_list_second_ptr = linked_list_ptr->second_list->next; //skip the dummy node.
			linked_list_second_ptr = linked_list_ptr->second_list;
			//printf("Car id: %d, Car name: %s , with section: ", linked_list_ptr->id, linked_list_ptr->name); 
			while(linked_list_second_ptr != NULL) //second list traverse.
			{
				...
				val_buf = IArrSearch(flag_list, msg_num, linked_list_second_ptr->id); //search the mapped index.
				// val_buf = linked_list_second_ptr->id;
...
struct linked_list *getVehSect(struct linked_list *veh_list) //extract all the sections vehicles passed. Return the head of the section.
{
	...
	while(v_ptr != NULL) //iterate in vehicle linked list.
	{
		...
		{
			sect = snd_ptr->id;
			if((s_ptr = findListId(sect_head, sect)) == NULL)
			{
