...
#define BEGN_TIME 23000
#define END_TIME 32000
...
int GetBigMap(double x, double y); //return the big map index according to (x,y).
...
void goToTime(FILE* arg_file, int time_step); //go to the specific time step in the file.
struct linked_list* parseSect(FILE* arg_file); //parse all the section passed between BEGN_TIME and END_TIME. Return NULL if failed.

FILE *fl_input = NULL, *fl_input_map = NULL, *fl_output = NULL, *fl_output_map = NULL;
void setRSUCORD(); //set the coordinate of the RSUs by reading file RSU_CORD.txt.
char time_now[50] = "";
int last_time = 0;
int time_val = 0;
int mapping_table[2933][3];
...
int main(int argc, char **argv)
{
	...
	*/
		...
		if(fl_input == NULL)  // my_fcd.xml
		{
			fprintf(stderr, "!!!!!	Dataset file with Path	%s	is failed to open. !!!!!\n", *(argv + 1));
			exit(1);
		}
		fl_input_map = fopen(*(argv + 2), "r"); //open input & output files.
		if(fl_input_map == NULL)  // my_fcd.xml
		{
			fprintf(stderr, "!!!!!	Dataset file with Path	%s	is failed to open. !!!!!\n", *(argv + 1));
			exit(1);
		}

		fl_output = fopen(*(argv + 3), "w+");
		if(fl_output == NULL)  // map_collect.txt
		{
			fprintf(stderr, "!!!!!	WARNING: Output map_collect.txt file with Path	%s	is failed to open. !!!!!\n", *(argv + 2));
			fl_output = stdout;
		}
		fl_output_map = fopen(*(argv + 4), "w+");
		if(fl_output_map == NULL)  // map.txt
		{
			fprintf(stderr, "!!!!!	WARNING: Output map.txt file with Path	%s	is failed to open. !!!!!\n", *(argv + 2));
		...
		printf("Enter the iters: ");
		scanf("%d", &iter_); //get the time interval we should parse.
		...
		while((ptr = fgets(line, 1024, fl_input_map)) != NULL){
			const char* d = ",";
			char *p;
			int temp[3] = {0, 1, 2};
			p = strtok(ptr, d);
			...
			int num = 0;
			while (p != NULL) {
				mapping_table[mapping_num][num] = atoi(p);
				num ++;
				p = strtok(NULL, d);	
			}
			mapping_num ++;
		...
		for(int i = 0; i<2933;i++){
			printf("%d ", mapping_table[i][1]);
		}*/	

		//goToTime(fl_input, BEGN_TIME);
...
void giveNewSmallMap(int target, int *new_small, int *new_big){
	for(int i = 0; i<2933; i++){
		if(mapping_table[i][0] == target){
			*new_small = mapping_table[i][1];
			*new_big = mapping_table[i][2];
		}
	}
}
		...
		*/
		...
		while(linked_list_ptr != NULL) //first list traverse.
		{
			...
			while(linked_list_second_ptr != NULL) //second list traverse.
			{
				...
				giveNewSmallMap(linked_list_second_ptr->id, &val_buf, &big_val_buff);
				
				/* Get new Big map index */
				//org_big_target = GetBigMap(linked_list_second_ptr->x, linked_list_second_ptr->y); // org_big_target is the original big map index
...
int GetBigMap(double x, double y) //return the big map index according to (x,y).
{
	int sect_per_row = (X_END - X_START) / 20;
	int big_sect_id = 0;

	big_sect_id = ((int) (x - X_START) / 20) + sect_per_row * (int) ((Y_START - y) / 20); //calculate the section id.
	if(big_sect_id < 0)
	{
		printf("***** Warning: Negative big_sect_id %d\n. *****", big_sect_id);
		return big_sect_id;
	}
	return big_sect_id;
}
...
struct linked_list *ChangeInToBig(struct linked_list *sect_list){
	...
	while(v_ptr != NULL) 
	{
		...
		sect = GetBigMap(v_ptr->x, v_ptr->y);
...
void *ListForFlag(struct linked_list *veh_list, struct linked_list *flag_list_head, struct linked_list *flag_list_tail ){
	...
	while(v_ptr != NULL) //iterate in vehicle linked list.
	{
		...
		if((s_ptr = findListId(flag_list_head, sect)) == NULL)  // NOT duplicate then save
		{
			...
			flag_list_tail->x = v_ptr->x;
			flag_list_tail->y = v_ptr->y;
		}
		v_ptr = v_ptr->next;
	}
...
struct linked_list *getVehSect(struct linked_list *veh_list) //extract all the sections vehicles passed. Return the head of the section.
{
	...
	while(v_ptr != NULL) //iterate in vehicle linked list.
	{
		...
		while(snd_ptr != NULL) //iterate in the section list of each vehicle.
		{
			...
			{
				sect_tail = listIdInsert(&sect_head, sect_tail, sect);
				sect_tail->x = snd_ptr->x;
				sect_tail->y = snd_ptr->y;
				sect_tail->val ++;
			}
			else
			{
...
struct linked_list *parseVehSect(FILE *fl_ptr, struct linked_list *veh_head, int time) //parse the section that the vehicles will pass in next 'time' seconds. Return vehicle linked list if success, else NULL.
{
	...
	for(int i = 0; i < time; i ++) //times iteration.
	{
		...
		{
			if(strstr(ptr, "</timestep>") != NULL) //when meet the end of the time step, hold the parsing.
			{
				break;
			}
			...
			if((veh_ptr = findListName(veh_head, name_ptr)) != NULL) //parse the passing section only when the vehicle exists.
			{
				x = parseVehX(ptr);
				y = parseVehY(ptr);
				sect = ((int) (x - X_START) / 10) + sect_a_row * (int) ((Y_START - y) / 10); //calculate the section id.
				...
				if(findListId(veh_ptr->second_list, sect) == NULL)
				{
					...
					veh_ptr->second_list_tail->x = x;
					veh_ptr->second_list_tail->y = y;
				}
			}
		}
	...
	while((ptr = fgets(line, 1024, fl_ptr)) != NULL) //get lines.
	{
		...
		if(parseVehName(line, name) != NULL) //when we find the vehicle, check for its coordinate, and decide whether should insert it or not.
		{
			...
			if(dist < radXrad) //insert when the vehicle is covered by RSU.
			{
				...
				veh_tail->x = x;
				veh_tail->y = y;
				sect = ((int) (veh_tail->x - X_START) / 10) + sect_a_row * (int) ((Y_START - veh_tail->y) / 10); //calculate the section id.
				veh_tail->second_list_tail = listIdInsert(&(veh_tail->second_list), veh_tail->second_list_tail, sect); //insert the sect into veh node.
				veh_tail->second_list_tail->x = x;
				veh_tail->second_list_tail->y = y;
			}
		}
	}
...
void goToTime(FILE* arg_file, int time_step) //go to the specific time step in the file.
{
	char line[1024] = "\0";
	char* char_ptr = NULL;
	int time_step_len = strlen("<timestep time=\"\0");

	for(int cntr = 0; cntr < time_step; cntr ++) //speed up this time-finding process.
	{
		char_ptr = fgets(line, 1024, arg_file);
	}

	while((char_ptr = fgets(line, 1024, arg_file)) != NULL) //find out the line with <time_step> string.
	{
		if((char_ptr = strstr(line, "<timestep time=\"\0")) != NULL)
		{
			char_ptr = char_ptr + time_step_len;
			if(atoi(char_ptr) >= time_step)
			{
				//printf("Target time step: %d, acquired line of the time step: %s\n", time_step, line); //for testing.
				break;
			}
		}
	}
...
struct linked_list* parseSect(FILE* arg_file) //parse all the section passed between BEGN_TIME and END_TIME. Return NULL if failed.
{
	struct linked_list* sect_list = NULL;
	struct linked_list* sect_list_tail = NULL;
	char line[1024] = "\0";
	char* char_ptr = NULL;
	double x = 0.0, y = 0.0;
	int sect = 0;
	int sect_per_row = (X_END - X_START) / 10;
	int time_begin_len = strlen("<timestep time=\"");
	
	while((char_ptr = fgets(line, 1024, arg_file)) != NULL) //scan all of the lines in the input file.
	{
		if((char_ptr = strstr(line, "<timestep")) != NULL) //encounter the begin of a time step.
		{
			char_ptr = char_ptr + time_begin_len;
			if(atoi(char_ptr) > END_TIME) //encounter the end of the demanding time step.
			{
				return sect_list;
			}
			else
		...
		else if(strstr(line, "</timestep") != NULL) //encounter the end of a time step.
		{
			continue;
		}
		...
		if(x == CORD_MAX || y == CORD_MAX) //error checking.
		{
			fprintf(stderr, "***** Error: x = %lf, y = %lf are invalid in line: \n%s. *****\n", x, y, line);
			return NULL;
		}
		else
		{
			sect = ((int) (x - X_START) / 10) + sect_per_row * (int) ((Y_START - y) / 10); //calculate the section id.
			if(findListId(sect_list, sect) == NULL)
			{
				sect_list_tail = listIdInsert(&sect_list, sect_list_tail, sect); //insert the sect into veh node.
				sect_list_tail->x = x;
				sect_list_tail->y = y;
			}
		}
