...
class Gurobi:
    def __init__(self, fc1_weight, fc1_bias, fc2_weight, fc2_bias, vec_num, vec_num_plus_one, t_minus_s_, Reward_hit, Reward_fetch,
        ...
        self.vec_num_plus_one = vec_num_plus_one
    ...
    def MIP_formulation(self):
        ...
        for i in range(self.hidden_dim):  # self.hidden_dim:H
            ...
            m.addConstr(y_h[i] >= quicksum(self.f1_w[i][j] * (1 - s_[j]) for j in range(self.Total_map)) +
                        quicksum(self.f1_w[i][j + self.Total_map] * self.vec_num_plus_one[j] for j in range(self.Small_number)) +
                        self.f1_b[i], "c1_")

            m.addConstr(y_h[i] <= quicksum(self.f1_w[i][j] * (1 - s_[j]) for j in range(self.Total_map)) +
                        quicksum(self.f1_w[i][j + self.Total_map] * self.vec_num_plus_one[j] for
