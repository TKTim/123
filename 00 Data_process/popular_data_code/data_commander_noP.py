...
def get_value(count, for_p_temp):
...
def cal_popularity(num_, D3_value_car):  # D3_value_car
    for j in range(car_num_max):
        ...
        if count == 0:
            ...
            pri_ = get_value(count, for_p_temp)
...
D3_value = [[[[-1, -1] for k in range(max_time)] for j in range(car_num_max)] for i in range(iter_)]
...
Start_load = False
num_ = [[-1 for _ in range(max_time)] for _ in range(car_num_max)]
car_num = 0
iter_ = 0
for line in lines:
    pos = 0
    ...
    for i in line:
        if Start_load:
            if i == "E":  # End of a iter
                ...
                cal_popularity(num_, D3_value[iter_])
                # print(num_)
                num_ = [[-1 for _ in range(max_time)] for _ in range(car_num_max)]
                iter_ += 1
                break
...
print("small_map_number", small_map_number)
