...
class Gurobi:
    ...
    def MIP_formulation(self):
        ...
        # Set objective
        m.setObjective(quicksum(self.vec_num[self.time_round][i] * hit[i] * 1 for i in range(625)) +
                       quicksum(self.vec_num[self.time_round][i] * fetch[i] * 10 for i in range(625)) +
        ...
        for i in range(16):  # 16:H
            try:
                ...
                m.addConstr(y_h[i] >= quicksum(self.f1_w[i][j] * x_[j] for j in range(794)) + self.f1_b[i])
                # self.M_minus =
        ...
        for i in range(794):
            try:
                ...
                # m.addConstr(d_m_[625+getBigmap(i)] == s_[625+getBigmap(i)] - self.t_minus_x_[625+getBigmap(i)])
                # m.addConstr(d_m_[i] >= 0)
                # m.addConstr(d_m_[625+getBigmap(i)] >= 0)
        ...
        # cons. 7~8
        try:
            m.addConstr(quicksum(d_m_[i] for i in range(794)) <= self.D_size)
            m.addConstr(quicksum(s_[i] for i in range(794)) <= self.B_size)
        except:
            print("Fail sum")
...
t_minus_s_ = [0 for _ in range(794)]
...
U_ = 100
L_ = -100
vec_num = [[1.0 for _ in range(625)] for _ in range(1000)]  # numbers of maps

# W and B Random for test

fc1_weight = [[random.random() for _ in range(794)] for _ in range(16)]
...
'''
fc1_weight = [[0 for _ in range(794)] for _ in range(16)]
