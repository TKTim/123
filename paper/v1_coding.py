...
class DQN(object):
    def __init__(self):
        self.eval_net, self.target_net = Net(input_dim, hidden_dim, 1, batch_size), Net(input_dim, hidden_dim, 1,
                                                                                        batch_size)

        self.learn_step_counter = 0  # for target updating
        self.memory_counter = 0  # for storing memory
        self.memory_s = np.zeros((MEMORY_CAPACITY, N_S_A))  # initialize memory_s
        self.memory_a = np.zeros((MEMORY_CAPACITY, N_S_A))  # initialize memory_a
        self.memory_r = np.zeros((MEMORY_CAPACITY, 1))  # initialize memory_r
        self.memory_s_ = np.zeros((MEMORY_CAPACITY, N_S_A))  # initialize memory_s_
    ...
    def learn(self):
        ...
        b_memory_s = self.memory_s[sample_index, :]
        b_memory_a = self.memory_a[sample_index, :]
        b_memory_r = self.memory_r[sample_index, :]
        b_memory_s_ = self.memory_s_[sample_index, :]
        b_s = torch.FloatTensor(b_memory_s)
        b_a = torch.LongTensor(b_memory_a)
        b_r = torch.FloatTensor(b_memory_r)
        b_s_ = torch.FloatTensor(b_memory_s_)
