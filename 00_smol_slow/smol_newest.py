...
class DQN(object):
    ...
    def learn(self):
        ...
        b_s_ = torch.FloatTensor(b_memory[:, N_S_A * 2 + 1:N_S_A * 3 + 1])  # batch * |b_s_|
        g_step = torch.FloatTensor(b_memory[:, -1:])  # batch * 1
        print("g_step:\n", g_step, file=py_out)
...
def GetTargetS_(s_new, target_net, vec_num_in, g_step):
    ...
    for i in range(BATCH_SIZE):
        ...
        if game_step == GAME_STEP_NUM:
            ...
            game_step += 1
            gurobi_func = gb.Gurobi(fc1_weight, fc1_bias, fc2_weight, fc2_bias, vec_num_in[int(game_step)],
